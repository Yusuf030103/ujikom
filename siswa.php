...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
      ...
      echo '<div class="row">';
		...
		echo '<tr class="info"><th>#</th><th width="100">NIS</th><th>Nama Lengkap</th><th>Kelas</th><th>Prodi</th><th>Aksi</th></tr>
			 ';
		if( mysqli_num_rows($sql) > 0 ){
			$no = 1;
			while(list($nis,$nama,$idprodi,$kelas) = mysqli_fetch_array($sql)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$nis.'</td>';
				echo '<td>'.$nama.'</td>';
				echo '<td>'.$kelas.'</td>';
				$qprodi = mysqli_query($koneksi,"SELECT prodi FROM prodi WHERE idprodi='$idprodi'");
			...
			if( $_SESSION['admin'] == 1 ){
			echo '<a href="./admin.php?hlm=master&sub=siswa&aksi=baru" class="btn btn-primary" btn-xs">Tambah Data</a>';
			}
		} else {
